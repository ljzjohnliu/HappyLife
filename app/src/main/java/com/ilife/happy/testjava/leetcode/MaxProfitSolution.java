...
//        int[] prices = {2, 4, 1};
    ...
    public static int maxProfit(int[] prices) {
        int[] sortPrices = prices;
        Arrays.sort(sortPrices);


        int rightPrice = prices[0];
        int result = 0;
        for (int i = 0; i < prices.length; i++) {
            if (prices[i] < rightPrice) {
                rightPrice = prices[i];
            } else {
                if (result < prices[i] - rightPrice) {
                    result = prices[i] - rightPrice;
                }
            }
        }
        return result;
        ...
        for (int i = prices.length - 1; i >= 0; i--) {
            for (int j = i - 1; j >= 0; j--) {
                System.out.println("----prices[" + i +"] = " + prices[i] + ", prices[" + j +"] = " + prices[j]
                        + ", gap = " + (prices[i] - prices[j]) + ", result = " + result);
